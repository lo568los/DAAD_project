...
def circuit_3(N, trotter_steps, angles=0, theta_k=0, theta_z=0, num_cl_bits=0, trotter_barriers=False, save=False, initial_state=None):
    """
    Constructs the circuit. 
    If initial_state is provided (as a vector), it initializes the circuit with that state.
    Otherwise, it defaults to the Fermi Sea + Impurity Flip state.
    """
    if initial_state is not None:
        # Initialize with the exact ground state (includes impurity state)
        qc = QuantumCircuit(num_qubits, num_cl_bits)
        qc.initialize(initial_state, range(num_qubits))
        qc.barrier()
        # No manual X(N) here because initial_state already contains the correct impurity state.
    else:
        # Default initialization (Fermi Sea + Impurity Flip)
        if num_cl_bits == 0:
...
def generate_thermal_reference(eigvals, beta_list):
    """
    Computes Energy vs Beta curve using pre-computed eigenvalues.
    """
    print(f"Generating thermal reference from eigenvalues...")
    ...
    for beta in beta_list:
        ...
        # We use original eigvals for the weighted sum
...
# --- 1. Compute Exact Ground State and Spectrum ---
print("Diagonalizing Hamiltonian for Ground State and Bandwidth...")
# This gives us the exact Ground State vector AND the spectrum for thermal ref
eigvals_exact, eigvecs_exact = la.eigh(H_total_op.to_matrix())

ground_state_vector = eigvecs_exact[:, 0]
ground_state_energy = eigvals_exact[0]
...
# --- 2. Generate Thermal Reference Data ---
...
# Pass the pre-computed eigenvalues
energy_ref_list = generate_thermal_reference(eigvals_exact, beta_ref_list)

t_therm_end = time.time()
print(f"Thermal reference generated in {round(t_therm_end - t_therm_start, 2)}s")


# --- 3. Generate Circuits with Ground State Initialization ---
...
# Initialize with the exact ground state
qc_list2[0] = circuit_3(N, 0, theta, theta_k, theta_z, initial_state=ground_state_vector)
...
# --- 4. Execute Multithreaded Simulation (Existing Logic) ---
...
try:
    ...
    kondo_temp = D * np.exp(-1 / (rho * abs(theta_k)))
except:
    kondo_temp = 0
...
filename = f"N{N}_th{theta}_thk{theta_k}_teffgs.txt"
...
# Save plot
plot_filename = f"N{N}_th{theta}_thk{theta_k}_gsplot.png"
