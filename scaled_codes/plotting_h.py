...
h_vals2 = [0]*max_trotter_steps

for i in range(max_trotter_steps):
    data = np.loadtxt(f"../scaled_codes/data2/N = {N}, theta = {theta}, theta_k = {theta_k}, t = {i}_h_tol.txt")
    #print(data)
    h_vals[i] = data[1]/N
    data = np.loadtxt(f"../scaled_codes/data2/N = 10, theta = {theta}, theta_k = {theta_k}, t = {i}_h_tol.txt")
    #print(data)
    h_vals2[i] = data[1]/10

    #data = np.loadtxt(f"scaled_codes/data/N = 6, theta = {theta}, theta_k = {theta_k}, t = {i}_h.txt")
    #h_vals2[i] = data[1]/6
...
plt.plot(range(max_trotter_steps),h_vals,"r-", label = "N=6, FS Isotropic")
#plt.plot(range(max_trotter_steps),h_vals1, label = "N=10, old")
plt.plot(range(max_trotter_steps),h_vals2,"b-", label = "N=10, FS Isotropic")
plt.ylim(-2,0)
...
plt.savefig(f"H_plot_FS_panel3,theta=pi4,theta_k=pi6_tol2", dpi =1000)
plt.close()
